...
function getUser(id) {
    const sql = `SELECT HAB_CONECTIVIDADE, HAB_CASA_INTELIGENTE, HAB_ELETRICA, HAB_AUDIO_VIDEO,HAB_PABX_VOIP, HAB_METALICO FROM dbo.Tecnico WHERE ID_COLABORADOR = '${id}'`;
    const results = sqlServer.dispatchQuery(sql)
    return results;
}    
